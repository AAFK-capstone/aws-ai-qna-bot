...
module.exports=async function query(req,res) {
    ...
    let autoexit = _.get(req, "_settings.AUTO_EXIT_SPECIALTY_BOT_WHEN_ANSWER_NOT_FOUND", false);
    let fallbackOnFailedAnswer = _.get(req, "_settings.FALLBACK_TO_MAIN_BOT_WHEN_ANSWER_NOT_FOUND", false);

    if (specialtyBot) {
        console.log('Handling specialtyBot');
        let resp = await specialtyBotRouter.routeRequest(req, res, specialtyBot, specialtyBotAlias);
        if (fallbackOnFailedAnswer && _.has(resp, "res.session.qnabotcontext.specialtySessionAttributes.qnabot_gotanswer")) {
            if (resp.res.session.qnabotcontext.specialtySessionAttributes.qnabot_gotanswer === "false") {
                chainingConfig = true;
                _.set(resp, 'res.session.qnabotcontext.specialtyBotProgress', 'Failed')
                console.log("No answer received. Conditional chaining set to true. SpecialtyBotProgress as Failed." + resp)
                if (autoexit) {
                    delete resp.res.session.qnabotcontext.specialtyBot;
                    delete resp.res.session.qnabotcontext.specialtyBotName;
                    delete resp.res.session.qnabotcontext.specialtyBotAlias;
                    delete resp.res.session.qnabotcontext.specialtySessionAttributes;
                }
            }
        }
        if (resp.res.session.specialtyBotProgress === 'Complete' ||
            resp.res.session.specialtyBotProgress === 'Failed' || 
            resp.res.session.qnabotcontext.specialtyBotProgress === 'Failed') {
