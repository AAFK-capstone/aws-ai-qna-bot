 ...
 * helper function used for `get_parameter`
 * @param str
 * @returns boolean
 */
function isJson(str) {
...
/**
 * Function that changes setting in string to that in boolean
 * E.g. settings = ["true", "false", "True", "FALSE"] -> [ true, false, true, false ]
 ...
 * This Lambda function synchronises Kendra with new FAQ content and is invoked by an event, 
 * which contains an S3 object which has been updated. The function uploads the FAQ to S3, 
 * then parses the JSON (using parseJSON.js) into Kendra's formatting and output it into Lambda's tmp folder in this file: '/tmp/qna_FAQ.json'
 * All the while, it continues to update the status of the file upload ('Creating FAQ', 'Sync Complete', 'Error') by updating 
 * the file: 'status/qna-kendra-faq.txt' in S3_OUTPUT_BUCKET using the `update_status` function.
 * @param event
 * @param context
 * @param cb
...
/**
 * Function used to set the status of synchronising Kendra with the new FAQs
 * helper function used in `performSync`
 * @param bucket 
 * @param {*} new_stat 
 * @returns 
 */
