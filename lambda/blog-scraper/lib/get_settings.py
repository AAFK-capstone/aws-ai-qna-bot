...
def str2bool(old_settings):
    new_settings = {k: convert_string_to_bool(
        v) for k, v in old_settings.items()}
    return new_settings
...
def get_all_settings():
    ...
    logger.info("Getting Default QnABot settings from SSM Parameter Store: {}".format(
        default_settings_param))
    default_settings = get_parameters(default_settings_param)

    logger.info("Getting Custom QnABot settings from SSM Parameter Store: {}".format(
