...
async function routeKendraRequest(event, context) {
    ...
    let showFAQLink = event.req["_settings"]["ALT_SEARCH_KENDRA_FAQ_SHOW_LINK"];
    let showHelpfulLinks = _.get(event.req,"_settings.ALT_SEARCH_KENDRA_FAQ_SHOW_HELPFUL_LINKS");
    let showHelpfulLinksText = event.req["_settings"]["ALT_SEARCH_KENDRA_FAQ_SHOW_HELPFUL_LINKS_TEXT"];
    let showHelpfulLinksMsg = event.req["_settings"]["ALT_SEARCH_KENDRA_FAQ_HELPFUL_LINKS_MESSAGE"];
    let speechMessage = "";
    ...
    var answerTextMd
    var debug_results = [];
    // resArray contains results from different Kendra Indexes
    resArray.forEach(function (res) {

        if (res && res.ResultItems.length > 0) {
            res.ResultItems.forEach(function (element, i) {
                // if result item is less than confidence filter, ignore it
                if(!confidence_filter(minimum_score,element)){
                    return;
                }
                // allow kendra to search all results, instead of returning after finding top if showHelpfulLinks is true
                // else if showHelpfulLinks is false, end after finding top answer
                if(seenTop && !showHelpfulLinks){
                    return;
                }
                ...
                if (element.Type === 'ANSWER' && foundAnswerCount === 0 && element.AdditionalAttributes &&
                ...
                } else if (element.Type === 'QUESTION_ANSWER' && seenTop == false && element.AdditionalAttributes && element.AdditionalAttributes.length > 1) {
                    // only if top answer is not found, otherwise only search for Document type
                    ...
                    let answerMsgSource = "";
                    let questionText = element.AdditionalAttributes[0].Value.TextWithHighlightsValue.Text.replace(/\r?\n|\r/g, " ");
                    if (showFAQLink && element.DocumentURI) {
                        answerMsgSource += `\n\n ${helpfulLinksMsg}: `;
                        if (questionText) {
                            answerMsgSource += `<span translate=no>[${questionText}](${element.DocumentURI})</span>`
                        } else {
                            answerMsgSource += `<span translate=no>(${element.DocumentURI})</span>`
                        }
                    }
                    answerMessageMd = faqanswerMessageMd + '\n\n' + answerTextMd + answerMsgSource;
                    ...
                    const docInfo = {}
                    // if topAnswer found, then do not show document excerpts unless showHelpfulLinksText is set to true
                    if (seenTop == false || showHelpfulLinksText) { // configure the document excerpts
        ...
        answerDocumentUris.forEach(function(element) {
            ...
            if (element.DocumentURI) {
                ...
                if (element.DocumentTitle.Text) {
                    ...
                    markdown += `<span translate=no>(${element.DocumentURI})</span>`
                }
            }
        });
    }
    // condition to show helpful links when seenTop == true (i.e. when there is already a top answer)
    let idx = -1;
    if (seenTop == false) {
        idx = foundAnswerCount;
    } else if (showHelpfulLinks && foundDocumentCount > 0 && foundAnswerCount > 0) {
        idx = 1; //FAQ answer count as 1
        if (showHelpfulLinksMsg) {
            markdown += `\n\n`;
            markdown += `***`;
            markdown += `\n\n <br>`;
            markdown += `\n\n ${showHelpfulLinksMsg}: \n\n`;
