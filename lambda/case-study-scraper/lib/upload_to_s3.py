...
default_case_study_url = 'https://aws.amazon.com/api/dirs/items/search?item.directoryId=customer-references&sort_by=item.additionalFields.sortDate&sort_order=desc&size=94&item.locale=en_US'
...
'''
def generate_json_metadata(max_case_study, filename, bucketname, prefix, tag_delimiter=' ; ', slice=None):
    ...
    for index, row in tagDataframe.iterrows():
        ...
        if uploadCount['success'] >= max_case_study:
            uploadSuccess = True
            break
