...
#function definition
def get_case_study(localfilename, url):
    link_list = []
    ...
    for item in item_list:
        ...
        link_map = {'id': id, 'title': title, 'link': url, 'industry' : industry, 'location': location, 'contentType' : contentType}
    ...
    with open(localfilename, 'w', newline='') as csvfile:
        fieldnames = ['id', 'title', 'url', 'industry', 'location', 'contentType', 'use-case', 'product', 'tech-category', 'customer-segment']
        ...
        for link in link_list:
            for keyname in fieldnames:
                if keyname not in link:
                    link[keyname] = ''
            writer.writerow({'id': link['id'], 'url': link['link'], 'title': link['title'], 'industry': link['industry'], 'location': link['location'], 'contentType': link['contentType'], 'use-case': link['use-case'], 'product': link['product'], 'tech-category': link['tech-category'], 'customer-segment': link['customer-segment']})
...
if __name__ == '__main__':
    ...
    get_case_study(
        '../../kendra/csvFile.csv',
        'https://aws.amazon.com/api/dirs/items/search?item.directoryId=customer-references&sort_by=item.additionalFields.sortDate&sort_order=desc&size=94&item.locale=en_US&tags.id=GLOBAL%23industry%23financial-services&tags.id=GLOBAL%23tech-category%23storage'   
    )
