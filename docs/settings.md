...
| ALT_SEARCH_KENDRA_FAQ_SHOW_LINK | true or false | Allows Chatbot to show the source link of the FAQ answer
| ALT_SEARCH_KENDRA_FAQ_SHOW_HELPFUL_LINKS | true or false | Allows Chatbot to list Kendra Document search answers (source link only) when Kendra FAQ gives an answer
| ALT_SEARCH_KENDRA_FAQ_SHOW_HELPFUL_LINKS_TEXT | true or false | Allows Chatbot to provide text responses of matching search keywords, on top of the Kendra Document search answers when Kendra FAQ gives an answer (ALT_SEARCH_KENDRA_FAQ_SHOW_HELPFUL_LINKS must be set to true for this to work)
| ALT_SEARCH_KENDRA_FAQ_HELPFUL_LINKS_MESSAGE | text | Shows user-defined Message when FAQ answers are showed with document search answers
